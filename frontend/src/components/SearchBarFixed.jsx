...
export default function SearchBar({ products = [], onFilterChange }) {
  ...
  // ✅ MAIN SEARCH HANDLER - Used by both button and Enter key
  const handleSearch = () => {
    ...
    if (!query) {
      toast.error(
        t("দয়া করে পণ্যের নাম প্রবেশ করুন।", "Please enter a product name.")
      );
      return;
    }

    // 2. Case-insensitive search for exact/partial match
    ...
    if (foundProduct) {
      ...
      console.log("❌ Product not found for query:", query);
      toast.error(t("কোনো পণ্য পাওয়া যায়নি।", "No product found."));
      navigate("/products/not-found", { state: { searchQuery: query } });
      setShowSuggestions(false);
    }
  };
  ...
  // ✅ Handle suggestion click - Navigate directly
  const handleSuggestionClick = (product) => {
    setSearchQuery(product.name);
    setShowSuggestions(false);
    console.log("✅ Suggestion clicked:", product.name, "ID:", product._id);
      ...
      <div className="relative group">
        ...
        <div className="relative">
          ...
          {searchQuery && (
            <button
              onClick={handleClear}
              className={`absolute right-16 top-1/2 transform -translate-y-1/2 p-1.5 rounded-full transition-all hover:scale-110 z-10 ${
                isDark
                  ? "text-slate-400 hover:text-slate-200 hover:bg-slate-700"
                  : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"
              }`}
              title={t("साफ करो", "Clear")}
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          {/* Search Button - ✅ MAIN FIX: Added onClick handler */}
          <button
            onClick={handleSearch}
            className={`absolute right-2 top-1/2 transform -translate-y-1/2 px-6 py-2.5 rounded-full font-semibold transition-all duration-300 hover:scale-105 shadow-md z-10 ${
              isDark
                ? "bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white"
                : "bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white"
            }`}
            title={t("অনুসন্ধান", "Search")}
          >
            {t("খুঁজুন", "Search")}
          </button>
        </div>
