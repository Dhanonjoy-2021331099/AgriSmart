...
export default function AddProductPage() {
    ...
    const loadProducts = async () => {
      try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
          ...
          throw new Error(
            "Server returned non-JSON response. Please check backend URL and ensure backend is running."
          );
        }
        ...
        if (!res.ok) {
          ...
          throw new Error(
            errorData.message || `পণ্যের তালিকা লোড করা যায়নি (${res.status})।`
          );
        }
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        ...
        if (text.includes("<!DOCTYPE") || text.includes("<html")) {
          errorMsg +=
            "Backend may be returning HTML error page. Check Vercel logs.";
        } else if (res.status === 0 || !res.status) {
          errorMsg +=
            "Cannot connect to backend. Check if backend is deployed and URL is correct.";
        } else {
      ...
      if (!res.ok) {
        throw new Error(
          responseData?.message ||
            responseData?.error ||
            `পণ্য যুক্ত করা যায়নি (${res.status})।`
        );
      }
  ...
  return (
    <div
      style={{ minHeight: "100vh", padding: "40px 20px", background: bgColor }}
    >
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ margin: 0, color: textSecondary, maxWidth: "620px" }}>
              আন্তর্জাতিক বাজারে পাঠানোর জন্য পণ্যের তথ্য দিন। ছবির URL যে কোনো
              মাপের হলেও প্রিভিউ বক্সে সঠিকভাবে দেখা যাবে।
            </p>
          </div>
          <Link
            to="/products"
        ...
        >
              ...
              {[
                {
                  ...
                  name: "name",
                  type: "text",
                  placeholder: "উদাহরণ: সুনামগঞ্জ ধান",
                },
                {
                  label: "ছবির লিংক (URL)",
                  name: "image",
                  type: "url",
                  placeholder: "https://example.com/rice.png",
                },
                {
                  label: "মূল্য (TK)",
                  name: "price",
          ...
          >
            <div>
              <p style={{ margin: 0, color: textMuted, fontWeight: 600 }}>
                লাইভ প্রিভিউ
              </p>
            ...
            >
              ...
              <div
                style={{
                  padding: "16px",
                  borderRadius: "16px",
                  background: isDark ? "#831843" : "#fdf2f8",
                }}
              >
                <p
                  style={{
        ...
        {(isLoading || addedProducts.length > 0) && (
          <section style={{ marginTop: "60px" }}>
            ...
            >
              <h2 style={{ margin: 0, color: textColor, fontSize: "30px" }}>
                যুক্ত করা পণ্য
              </h2>
              <span
                style={{
              ...
              >
                {addedProducts.map((product) => (
                  ...
                  >
                    ...
                    <div>
                      <h3 style={{ margin: "0 0 6px", color: textColor }}>
                        {product.name}
                      </h3>
